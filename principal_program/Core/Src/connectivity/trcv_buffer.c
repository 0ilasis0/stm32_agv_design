...
FnState connect_trcv_buf_push(ByteTrcvBuf* self, VecByte* vec_byte)
{
    ...
    FNS_ERROR_CHECK(vec_byte_realign(vec_byte));
    FNS_ERROR_CHECK(vec_byte_push(&self->vecs[tail], vec_byte->data + vec_byte->head, vec_byte->len));
    self->len++;
    return FNS_OK;
}
...
FnState connect_trcv_buf_pop(ByteTrcvBuf* self, VecByte* vec_byte)
{
    ...
    FNS_ERROR_CHECK(vec_byte_push(vec_byte, self->vecs[self->head].data, self->vecs[self->head].len));
    if (--self->len == 0)
    {
        self->head = 0;
    }
