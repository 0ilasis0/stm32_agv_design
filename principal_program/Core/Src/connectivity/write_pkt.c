...
FnState pkt_left_speed(VecByte* vec_byte)
{
    vec_rm_all(vec_byte);
    ERROR_CHECK_FNS_RETURN(vec_byte_push(vec_byte, (uint8_t[]){CMD_B0_DATA, CMD_B1_LEFT_SPEED, 0x01, 0x00}, 4));
    ERROR_CHECK_FNS_RETURN(vec_byte_push_f32(vec_byte, motor_left.speed_present));
    return FNS_OK;
}
...
FnState pkt_right_speed(VecByte* vec_byte)
{
    ...
    ERROR_CHECK_FNS_RETURN(vec_byte_push_f32(vec_byte, motor_right.speed_present));
    return FNS_OK;
}
...
FnState pkt_left_duty(VecByte* vec_byte)
{
    ...
    ERROR_CHECK_FNS_RETURN(vec_byte_push(vec_byte, (uint8_t[]){0x00, 0x00, 0x00}, 3));
    ERROR_CHECK_FNS_RETURN(vec_byte_push_byte(vec_byte, motor_left.duty_value));
    return FNS_OK;
}
...
FnState pkt_right_duty(VecByte* vec_byte)
{
    ...
    ERROR_CHECK_FNS_RETURN(vec_byte_push(vec_byte, (uint8_t[]){0x00, 0x00, 0x00}, 3));
    ERROR_CHECK_FNS_RETURN(vec_byte_push_byte(vec_byte, motor_right.duty_value));
    return FNS_OK;
}
#endif
